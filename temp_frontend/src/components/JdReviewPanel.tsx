...
import { FileText, User, Activity, Code2, Target, ShieldCheck, Vote, ArrowRight, Download, Loader2, Copy } from 'lucide-react';
...
export function JdReviewPanel({
    ...
    const handleGenerateMd = async () => {
        ...
        try {
            const mdObj = await api.generateJdMarkdown({
                title: editedJd.role || "未命名职位",
                key_responsibilities: [],
                required_skills: editedJd.stack || [],
                experience_level: editedJd.exp_level || "未指定",
                salary: { range: "", tax_type: "税前", has_bonus: false, description: editedJd.remarks || "" },
                work_location: "不限",
                bonus_skills: editedJd.plus_points || [],
                education: editedJd.education || "未指定",
                culture_fit: editedJd.culture_fit || []
            });
            setMarkdownResult(mdObj.markdown);
        } catch (err) {
            console.error(err);
            alert("JD 描述生成失败");
        } finally {
            ...
            <div className="flex-1 flex flex-col bg-[#1c1c1e]/40 backdrop-blur-[40px] rounded-[3.5rem] border border-white/[0.08] shadow-[0_48px_96px_-24px_rgba(0,0,0,0.8)] overflow-hidden relative z-10">
                ...
                <div className="flex-1 overflow-y-auto p-14 custom-scrollbar space-y-16">
                    <div className="grid grid-cols-2 gap-x-16 gap-y-12">
                        ...
                        <div className="space-y-4">
                            <label className="flex items-center gap-2.5 text-xs font-bold text-zinc-500 uppercase tracking-[0.2em] px-1">
                                <Activity className="w-4 h-4 text-zinc-400" /> 薪资详情及说明
                            </label>
                            <textarea
                                rows={2}
                                className="w-full bg-white/[0.04] border border-white/[0.05] rounded-[1.5rem] px-8 py-7 text-white text-xl leading-relaxed focus:bg-white/[0.08] focus:border-white/20 focus:ring-8 focus:ring-white/5 outline-none transition-all resize-none shadow-inner"
                                value={editedJd.remarks}
                    ...
                    {/* Markdown 生成及预览区域 */}
                    <div className="mt-10 p-10 rounded-[2.5rem] bg-black/40 border border-white/[0.06] flex flex-col gap-6 group hover:bg-black/50 transition-all duration-700">
                        <div className="flex items-center justify-between">
                            <div className="space-y-2">
                                <p className="text-sm font-bold text-white uppercase flex items-center gap-2">
                                    <FileText className="w-5 h-5 text-blue-400" /> 智能详尽文案
                                </p>
                                <p className="text-[13px] text-zinc-500 max-w-lg">
                                    调用 API 根据以上碎片化关键词，生成段落清晰、可直发招聘平台的招聘贴（Markdown 格式）。
                                </p>
                            </div>
                            <button
                                onClick={handleGenerateMd}
                                disabled={isGeneratingMd}
                                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-[#0071E3] hover:bg-[#0077ED] text-white font-bold tracking-widest text-sm shadow-[0_0_20px_rgba(0,113,227,0.3)] disabled:opacity-50 transition-all active:scale-95"
                            >
                                {isGeneratingMd ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileText className="w-4 h-4" />}
                                {isGeneratingMd ? '生成中...' : '生成招聘文案'}
                            </button>
                        </div>

                        {markdownResult && (
                            <div className="mt-4 animate-fadeInUp">
                                <div className="bg-[#1c1c1e] p-6 rounded-2xl border border-white/[0.05] text-sm text-zinc-300 whitespace-pre-wrap leading-relaxed max-h-[400px] overflow-y-auto custom-scrollbar font-sans shadow-inner">
                                    {markdownResult}
                                </div>
                                <div className="flex mt-4 gap-3 justify-end">
                                    <button onClick={handleCopyMd} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/10 text-zinc-300 hover:text-white hover:bg-white/5 text-xs font-bold uppercase transition-all">
