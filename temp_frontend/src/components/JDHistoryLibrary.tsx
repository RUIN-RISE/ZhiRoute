...
import { X, Clock, ChevronRight, FileText, Search, Plus } from 'lucide-react';
...
export function JDHistoryLibrary({ onClose, onSelectJD }: JDHistoryLibraryProps) {
    const [searchQuery, setSearchQuery] = useState('');
    const [records] = useState<JDHistoryRecord[]>(mockHistory);
            ...
            >
                ...
                <div className="h-20 border-b border-white/10 px-8 flex items-center justify-between shrink-0 relative z-10 bg-[#111111]/80 backdrop-blur-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-[#f5f5f7] tracking-tight">招聘项目历史库</h2>
                            <p className="text-xs text-[#86868b] mt-0.5">历史保存的 JD 与人才寻访配置集</p>
                        </div>
                    </div>
                ...
                <div className="px-8 py-5 flex items-center justify-between border-b border-white/5 relative z-10">
                    ...
                    <button className="flex items-center gap-2 px-4 py-2 bg-[#1c1c1e] hover:bg-[#2c2c2e] border border-white/10 rounded-full text-sm font-medium text-[#f5f5f7] transition-colors shadow-sm">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {filteredRecords.map((record, idx) => (
                            ...
                            >
                                ...
                                <div className="h-full flex items-center justify-center pl-6 border-l border-white/5 opacity-0 group-hover:opacity-100 transition-all -translate-x-4 group-hover:translate-x-0 shrink-0">
                                    <button className="flex items-center justify-center w-10 h-10 rounded-full bg-[#0071e3]/10 text-blue-400 hover:bg-[#0071e3] hover:text-white transition-all">
