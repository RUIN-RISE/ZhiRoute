...
interface UserProfilePanelProps {
    ...
    onSelectJD: (id: string | number) => void;
}
 ...
 */
export function UserProfilePanel({ userCode, onLogout, onLoginClick, isLoggedIn, jdHistory, onDeleteHistory, onSelectJD }: UserProfilePanelProps) {
    ...
    const recentCount = jdHistory.length;
                        ...
                        <div className="px-5 py-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                            {jdHistory.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                    {jdHistory.slice(0, 10).map((record, idx) => {
                                        const raw = typeof record.content === 'string' ? JSON.parse(record.content) : record.content;
                                        const roleTitle = raw?.title ?? raw?.role ?? '未知岗位';
                                        const snippet = raw?.remarks ?? raw?.salary?.range ?? '配置保存记录';
                                        ...
                                        return (
                                            <motion.div
                                                ...
                                                className="flex items-start gap-3 px-3 py-3 rounded-xl hover:bg-white/[0.03] transition-colors group cursor-pointer"
                                                onClick={() => {
                                                    setShowTimeline(false);
                                                    onSelectJD(record.id);
                                                }}
                                            >
                                                {/* 时间线竖线 & 圆点 */}
                                                <div className="flex flex-col items-center gap-1 pt-1 shrink-0">
                                                    <div className="w-2 h-2 rounded-full bg-blue-500" />
                                                    {idx < Math.min(jdHistory.length, 10) - 1 && (
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-xs font-semibold text-blue-400 truncate pr-2">
                                                            {roleTitle}
                                                        </span>
                                                        <span className="text-[10px] text-zinc-600 shrink-0">
                                                            {formatRelativeTime(new Date(record.created_at).getTime())}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center justify-between mt-0.5">
                                                        <p className="text-xs text-zinc-500 truncate flex-1">{snippet}</p>
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                if (confirm("确认删除该 JD 历史吗？")) {
                                                                    onDeleteHistory(record.id);
                                                                }
                                                            }}
                                                            className="ml-2 w-5 h-5 flex items-center justify-center text-zinc-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all shrink-0"
                                                            title="删除此记录"
                                                        >
                                                            <X className="w-3 h-3" />
                                                        </button>
                        ...
                        {jdHistory.length > 0 && (
                            <div className="px-5 py-3 border-t border-white/5 flex items-center justify-between">
                                <div className="flex items-center gap-1.5 text-zinc-500">
                                    ...
                                    <span className="text-[11px]">最多保留最新 10 条</span>
                                </div>
                            </div>
                        )}
                    </motion.div>
